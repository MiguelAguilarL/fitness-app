...
export default function RestTimerSection({ label, onSkipRest, progress = 1, disabled = false }) {
  ...
  const titleClassName = disabled ? 'text-[22px] font-semibold text-[#545a6d]' : 'text-[22px] font-semibold text-[#666d80]'
  const timerClassName = disabled ? 'text-[36px] font-black text-[#5b496f]' : 'text-[42px] font-black text-[#8f46ff]'
  ...
  const buttonTextClassName = disabled ? 'text-[18px] font-bold text-white/30' : 'text-[19px] font-bold text-[#8f46ff]'

  return (
    <View className={`mt-8 overflow-hidden rounded-[30px] px-5 py-5 ${mutedContainerClassName}`}>
      ...
      <View className={`mt-7 h-2.5 overflow-hidden rounded-full ${progressTrackClassName}`}>
      ...
      <Pressable
        onPress={onSkipRest}
        disabled={disabled}
        className={`mt-7 items-center justify-center rounded-full py-4 ${buttonClassName}`}
